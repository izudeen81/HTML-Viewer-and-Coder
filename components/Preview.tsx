 ...
 * and an Inspector mode to select elements and highlight them in the code editor.
...
const Preview: React.FC<PreviewProps> = ({ htmlCode, isWysiwygEnabled, isInspectorEnabled, onWysiwygChange, onElementSelect }) => {
  ...
  useEffect(() => {
    ...
    if (!isMounted.current) {
        ...
        // to avoid a full reload, which would lose user focus and cursor position in WYSIWYG mode.
        const doc = iframe.contentDocument;
        if (!doc) return;

        // FIX: The following logic robustly handles updates for full HTML documents,
        // snippets, or empty strings to prevent stale content in the preview.
        ...
        // Determine what content to use for the head and body.
        if (newBodyMatch) {
            // Found a <body> tag, so we assume it's a full document (or at least contains a body).
            newHeadHtml = newHeadMatch ? newHeadMatch[1] : ''; // If no head tag, this clears the head.
            newBodyHtml = newBodyMatch[1] || '';
        } else {
            // No <body> tag found. Assume the entire content is an HTML snippet for the body.
            newHeadHtml = ''; // Snippets shouldn't have a head, so we clear it.
            newBodyHtml = htmlCode;
        }
        
        // Apply the updates to the iframe document.
        if (doc.head && newHeadHtml !== doc.head.innerHTML) {
            doc.head.innerHTML = newHeadHtml;
        }
    
        // Only update the iframe's body if the content has actually changed.
        // This check is vital to prevent an infinite feedback loop from the WYSIWYG editor's oninput event.
  ...
  return (
    <iframe
      ...
      // - `allow-scripts`: Allows JavaScript to run inside the iframe. This is
      //   necessary for the previewed content to be interactive (e.g., blog post scroll progress).
      // - `allow-same-origin`: Allows the parent document (the app) to access
      //   the content of the iframe. This is essential for the WYSIWYG editor
      //   and the Element Inspector to function.
      // Note: This setup means code generated by Gemini can execute scripts.
      // As the user is editing their own code, this is an accepted risk,
      // similar to how CodePen or JSFiddle operates.
